...
from .schemas import FetchStaticOutput


def fetch_static(force: bool = False) -> FetchStaticOutput:
    """Fetch (download+extract) the static GTFS dataset if missing.

    Behavior:
      - Checks providers.yaml requested .txt files under configured static_dir.
      - If any are missing OR force=True: downloads and extracts again.
    """
    try:
        # Determine target static_dir and requested files using the existing reader config
        res = read_static_dir_from_yaml("src/config/providers.yaml", "provider.static")
        static_dir = res.static_dir

        # Inspect expected files from providers.yaml
        cfg = load_yaml("src/config/providers.yaml")
        files = cfg.get("provider.static.extract.files", []) or []
        expected = sorted({Path(str(f)).name for f in files if str(f).endswith(".txt")})

        present = {p.name for p in static_dir.glob("*.txt")} if static_dir.exists() else set()
        missing_before = sorted([f for f in expected if f not in present])

        if force or missing_before:
            rc = fetch_static_main()
            if rc != 0:
                return FetchStaticOutput(
                    ok=False,
                    static_dir=str(static_dir),
                    missing_before=missing_before,
                    missing_after=missing_before,
                    message=f"Static fetch failed with exit code {rc}.",
                )

        # Re-check after fetch
        present_after = {p.name for p in static_dir.glob("*.txt")} if static_dir.exists() else set()
        missing_after = sorted([f for f in expected if f not in present_after])

        ok = len(missing_after) == 0
        msg = "Static GTFS is ready." if ok else "Static GTFS fetched but some requested files are still missing."
        return FetchStaticOutput(
            ok=ok,
            static_dir=str(static_dir),
            missing_before=missing_before,
        ...
        return FetchStaticOutput(ok=False, static_dir="", message=f"Static fetch failed: {e!r}")
