...
def active_routes_with_vehicles() -> ActiveRoutesOutput:
    ...
    if not {"route_id", "vehicle_id"}.issubset(vp.columns):
        raise ValueError("vehicle_positions missing required columns route_id/vehicle_id")

    # Aggregate realtime: route_id -> unique vehicles
    grouped = (
        vp.dropna(subset=["route_id", "vehicle_id"])
        .assign(route_id=lambda d: d["route_id"].astype(str), vehicle_id=lambda d: d["vehicle_id"].astype(str))
        .groupby("route_id")["vehicle_id"]
        .unique()
    )

    # Load static routes for enrichment (if available)
    from src.sources.gtfs_static.tables import load_static_tables_from_yaml
    ...
    out = []
    for rid, vids in grouped.items():
        vids_list = sorted([str(v) for v in list(vids) if v is not None and str(v) != "nan"])
        ...
        out.append(
            ActiveRoute(
                ...
                vehicle_ids=vids_list,
                vehicles_active=len(vids_list),
            )
        )
