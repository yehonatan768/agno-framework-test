...
def _prepare_static_data() -> None:
    """Ensure static GTFS is available, without redownloading on every agent call."""
    cfg = load_yaml("src/config/providers.yaml")
    files = cfg.get("provider.static.extract.files", []) or []
    expected = sorted({str(Path(str(f)).name) for f in files if str(f).endswith(".txt")})

    repo = TransitRepository.from_paths_yaml("src/config/paths.yaml")
    static_dir = repo.paths.static_dir
...
def _prepare_realtime_data() -> None:
    """Fetch ONE realtime snapshot for this question and pin it via TRANSIT_SNAPSHOT_DIR."""
    # If caller already pinned a snapshot, keep it.
    if (os.getenv("TRANSIT_SNAPSHOT_DIR") or "").strip():
        return
    ...
    repo = TransitRepository.from_paths_yaml("src/config/paths.yaml")
    snap = repo.latest_snapshot_dir()
    if not snap:
        raise SystemExit("Realtime fetch completed but no snapshot directory was found.")

    # Pin snapshot for all tools executed as part of this question.
    os.environ["TRANSIT_SNAPSHOT_DIR"] = str(snap)
...
async def _run_team(args: argparse.Namespace, s: AppSettings) -> None:
    ...
    # Prepare data ONCE per question to keep all tool calls consistent.
    # - Static is only fetched if required files are missing.
    # - Realtime is fetched once and pinned via TRANSIT_SNAPSHOT_DIR.
    which = _classify(prompt)
    if which in {"planning", "both"}:
        _prepare_static_data()
    if which in {"execution", "both"}:
        _prepare_realtime_data()
